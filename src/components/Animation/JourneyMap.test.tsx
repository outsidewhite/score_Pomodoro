...
describe('到達したステージの地理地図', () => {
  ...
  it.each([0, 44, 45, 315, 359, 360, 855, 899, 900])('%i点で到達したステージだけを表示する', score => {
    render(<JourneyMap journey={journey} stops={getJourneyStops(journey, score, {})} />)
    expect(Boolean(screen.queryByRole('img', { name: '西から東への地図' }))).toBe(score >= 45)
    expect(Boolean(screen.queryByRole('img', { name: '王道世界一周の地図' }))).toBe(score >= 360)
    expect(Boolean(screen.queryByRole('img', { name: '月への旅の地図' }))).toBe(score >= 900)
  })
  ...
  it.each(worldRoutes)('$nameの地理座標を保ち、表示地点に合わせて余白を詰める', worldRoute => {
    ...
    const { rerender } = render(element(360))
