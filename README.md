...
```text
計測画面でMediaPipeモデルを準備（完了までタイマー開始不可）
    ↓
MediaPipeのランドマーク
    ↓
500msごとに肩位置・肩角度・人物検出状態を評価
    ↓
実時間1分を15秒×4区間に分け、姿勢・安定性・検出状態を0〜100点へ変換
    ↓
基準姿勢取得中は安定性6：検出4、取得後は姿勢4：安定性4：検出2で総合評価を計算
    ↓
総合評価を0〜3点の獲得スコアへ変換し、完了区間をsessionStorageへ保存
```

獲得スコアは総合評価0点で0点、1〜40点で1点、41〜80点で2点、81〜100点で3点です。15秒区間の検出状態が2回連続で0点になった場合は離席と判定し、未完了区間を破棄してタイマーを停止します。
